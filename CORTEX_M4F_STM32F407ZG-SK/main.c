...
static void GameTask( void *pvParameters )
{
	while( 1 ){
		GAME_Update();
		GAME_Render();
		vTaskDelay( RefreshSpeed );
	}
}


static void GamePowerBallTask( void *pvParameters )
{
	while( 1 ){
		GAME_PowerBall();
		vTaskDelay( 30 );
...
int main(void)
{
	...
	xTaskCreate( GamePowerBallTask, (signed char*) "GamePowerBallTask", 128, NULL, tskIDLE_PRIORITY + 1, NULL );
